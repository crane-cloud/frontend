...
const Search = ({
  ...
  const generateOptions = (type: string, items: any[] = []) => {
    const typeMap = {
      projects: {
        group: "Projects",
        path: (item: any) => `/projects/${item.id}`,
      },
      apps: {
        group: "Apps",
        path: (item: any) => `/projects/${item.project_id}/apps/${item.id}`,
      },
  ...
  const handleNavigation = (option: any) => {
    const navigators = {
      ...
      apps: () =>
        navigate(
          `/projects/${option.original.project_id}/apps/${option.original.id}`,
        ),
      users: () => {},
    };
