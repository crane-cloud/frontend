  ...
  renderErrors = () => (
    <span>
      ...
      <div className="dropdown-menu" aria-labelledby="dropdownMenu3">
        { /* errors dropdown */}
        {
          this.props.cluster.errors.map((error) => <button className="dropdown-item" type="button" key={error.error_id}> {error.error} </button>)
        }
      </div>
    </span>
  )
  ...
  renderNameSpaces = () => (
    <span>
      ...
      <div className="dropdown-menu" aria-labelledby="dropdownMenu4">
        {
          this.props.cluster.nameSpaces.map((namespace) => <button className="dropdown-item" type="button" key={namespace.nameSpaceId}>
            <Link to={{
