import handleProjectValidation, {
  validateDomain,
  validateName,
...
describe("checks valiidation", () => {
  it(" domain name errors ", () => {
    assert.equal(validateDomain(""), "Please provide a domain name");
    assert.equal(
      validateDomain("2john@"),
      "Use accepted formats, for example, google.com, yourdomain.ug"
    );
    assert.equal(validateDomain("johnr.com"), false);
  });
  it("validate names", () => {
  ...
  it("validating project name function", () => {
    ...
    assert.equal(
      validateProjectName("2na"),
      "Project name should start with a letter"
    );
    // assert.equal(validateProjectName('High low'),"Project name may only contain letters and a hypen -");
    assert.equal(
      validateProjectName(
        "Highndsjjdjnbhjvbhfdvbadjvbfdhvbbfvbvhfdbvhfdhdvjhsdjjdjds"
      ),
  ...
  it("validating project types errors", () => {
    assert.equal(
      validateProjectType("2najd"),
      "Project Type should start with a letter"
    );
    assert.equal(validateProjectType(""), "Project Type cannot be empty");
    assert.equal(
      validateProjectType("Hig"),
      "Project Type must be at least 4 characters"
    );
